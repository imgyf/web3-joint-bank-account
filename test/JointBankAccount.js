...
describe("JointBankAccount", function () {
  ...
  async function deployJointBankAccountWithAccounts(
    owners = 1,
    deposit = 0,
    withdrawlAmounts = []
  ) {
  ...
  describe("Depositing", () => {
    it("should allow deposit from account owner", async () => {
      const { bankAccount, addr0 } = await deployJointBankAccountWithAccounts(1);
      await expect(
    ...
    it("should NOT allow deposit from non-account owner", async () => {
      const { bankAccount, addr1 } = await deployJointBankAccountWithAccounts(1);
  ...
  describe("Withdraw", () => {
    describe("Request a withdraw", () => {
      it("account owner can request withdraw", async () => {
        const { bankAccount, addr0 } = await deployJointBankAccountWithAccounts(
          1,
          100
        );
      ...
      it("non-account owner cannot request withdraw", async () => {
        const { bankAccount, addr1 } = await deployJointBankAccountWithAccounts(
          1,
          100
        );
    ...
    describe("Approve a withdraw", () => {
      ...
      it("should not allow non-account owner to approve withdraw", async () => {
        const { bankAccount, addr2 } = await deployJointBankAccountWithAccounts(
          2,
          100,
          [100]
        );
    ...
    describe("Make withdraw", () => {
      it("should allow creator of request to withdraw approved request", async () => {
        const { bankAccount, addr0, addr1 } =
          await deployJointBankAccountWithAccounts(2, 100, [100]);
        await bankAccount.connect(addr1).approveWithdraw(0, 0);
        await expect(
      ...
      it("should not allow creator of request to withdraw approved request twice", async () => {
        const { bankAccount, addr0, addr1 } =
          await deployJointBankAccountWithAccounts(2, 200, [100]);
        await bankAccount.connect(addr1).approveWithdraw(0, 0);
        await expect(
          bankAccount.connect(addr0).withdraw(0, 0)
        ).to.changeEtherBalances([bankAccount, addr0], ["-100", "100"]);
